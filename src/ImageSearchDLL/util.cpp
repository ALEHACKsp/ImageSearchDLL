...
// Author: ImageSearch was created by Aurelian Maga.
{
#ifdef _DEBUG
	char buffer[256] = { 0 };
	sprintf(buffer, "left:%d ; top:%d ; right:%d ; buttom:%d ; file:%s", aLeft, aTop, aRight, aBottom, aImageFile);
	MessageBox(NULL, buffer, "Finding", MB_OK);
#endif
	...
	hbitmap_screen = GetScreenShot();
	if (   !(sdc = CreateCompatibleDC(hdc)) || (hbitmap_screen == INVALID_HANDLE_VALUE))
		goto end;
...
#ifdef _DEBUG
		char Title[128] = { 0 };
		sprintf(Title, "--%s", answer);
		MessageBox(NULL, answer, Title, MB_OK);
#endif
		return answer;
		//return "ZZ";
	}
